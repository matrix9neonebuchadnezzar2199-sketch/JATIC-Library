...
| **UI** | PySide6、4 タブ、ライト／ダーク、トースト、トレイ、DL 進捗 |
| **保管庫拡張** | ソート、再 DL、削除、タグ、月次 ZIP/CSV エクスポート |
| **分析** | カレンダー（欠損月）、2 か月比較、簡易交通量グラフ |
...
**配布 exe（予定）:** `dist/JATIC-Library/JATIC-Library.exe` — [残作業 R1〜R3](docs/DEV_STATUS.md#51-必須配布ブロッカー) 完了後に zip 配布。
...
3. **保管庫**タブで取得済み ZIP を閲覧（**カレンダー**・**比較**タブも利用可）
